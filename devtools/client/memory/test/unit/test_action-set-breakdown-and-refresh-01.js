...
/**
 * Tests the task creator `setBreakdownAndRefreshAndRefresh()` for breakdown
 * changing. We test this rather than `setBreakdownAndRefresh` directly,
 * as we use the refresh action in the app itself composed from
...
add_task(function*() {
  ...
  ok(isBreakdownType(getState().snapshots[1].census.report, "coarseType"),
    "Breakdown can be changed while saving snapshots, " +
    "uses updated breakdown in census");
  ...
  ok(breakdownEquals(getState().snapshots[2].census.breakdown,
                     breakdowns.allocationStack.breakdown),
    "Breakdown can be changed while saving census, " +
    "stores updated breakdown in snapshot");
  ...
  yield waitUntilState(store,
    () => isBreakdownType(getState().snapshots[2].census.report, "coarseType"));
  ok(isBreakdownType(getState().snapshots[2].census.report, "coarseType"),
    "Snapshot census updated when changing breakdowns after already " +
    "generating one census");

  dispatch(setBreakdownAndRefresh(heapWorker,
                                  breakdowns.allocationStack.breakdown));
  yield waitUntilState(store,
    () => isBreakdownType(getState().snapshots[2].census.report,
    "allocationStack"));

  // Does not update unselected censuses
  ok(!getState().snapshots[1].selected,
     "Second snapshot unselected currently");
  ok(breakdownEquals(getState().snapshots[1].census.breakdown,
     breakdowns.coarseType.breakdown),
    "Second snapshot using `coarseType` breakdown still and not yet updated " +
    "to correct breakdown");
  ok(isBreakdownType(getState().snapshots[1].census.report, "coarseType"),
    "Second snapshot using `coarseType` still for census and not yet updated " +
    "to correct breakdown");
  ...
  ok(getState().snapshots[1].selected,
     "Second snapshot selected currently");
  ok(breakdownEquals(getState().snapshots[1].census.breakdown,
                     breakdowns.allocationStack.breakdown),
    "Second snapshot using `allocationStack` breakdown and updated " +
    "to correct breakdown");
  ok(isBreakdownType(getState().snapshots[1].census.report, "allocationStack"),
     "Second snapshot using `allocationStack` for census and updated " +
     "to correct breakdown");
